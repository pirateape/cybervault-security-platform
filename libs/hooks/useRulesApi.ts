...
// Updated to use FastAPI backend instead of Next.js API routes
const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
const RULES_ENDPOINT = `${API_BASE}/rules`;
...
export function useRules(orgId: string) {
  return useQuery<RuleZ[], Error>({
    ...
    queryFn: async () => {
      const res = await fetch(`${RULES_ENDPOINT}/?org_id=${orgId}`);
...
export function useRule(orgId: string, ruleId: string) {
  return useQuery<RuleZ, Error>({
    ...
    queryFn: async () => {
      const res = await fetch(`${RULES_ENDPOINT}/${ruleId}?org_id=${orgId}`);
...
export function useRuleVersions(orgId: string, ruleId: string) {
  return useQuery<any[], Error>({
    ...
    queryFn: async () => {
      const res = await fetch(`${RULES_ENDPOINT}/${ruleId}/versions?org_id=${orgId}`);
...
export function useRestoreRuleVersion(orgId: string, ruleId: string) {
  ...
  return useMutation<any, Error, { versionId: string }>({
    mutationFn: async ({ versionId }) => {
      const res = await fetch(`${RULES_ENDPOINT}/${ruleId}/restore?org_id=${orgId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
